...
/*
 *  @author   : Dervac Technologies
 *  date    : 10 July, 2020
 *  SureExam (Online Exam Software)
 */
...
class Install extends CI_Controller {
  ...
  function step2($param1 = '', $param2 = '') {
    ...
    if ($param1 == 'configure_database') {
      ...
      if ($db_connection == 'failed') {
        redirect(site_url('install/step2/error_con_fail'), 'refresh');
      } else if ($db_connection == 'db_not_exist') {
        redirect(site_url('install/step2/error_nodb'), 'refresh');
      } else {
        // proceed to step 4
        session_start();
  ...
  function configure_routes() {
    ...
    $data_routes = str_replace('install',	'student',	$data_routes);
